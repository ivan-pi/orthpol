...
c
        call srecur(npm,1,0.,0.,a,b,ierr) 
        call drecur(npm,1,0.d0,0.d0,da,db,ierr)
c
c Compute the desired recursion coefficients.
c
        call sindp(n,m,a,b,epsma,alpha,beta,ierr,z,w,e,a1,b1)
...
c
   ...
   40 continue
      ...
      subroutine sindp(n,m,a,b,eps,alpha,beta,ierr,z,w,e,a1,b1)
c
...
c
   ...
   10 continue
      if(m.eq.0) return
      call sgauss(m,a,b,eps,z,w,ierr,e)
      do 30 imu=1,m
        mi=npm-imu
        do 20 k=1,mi+1
          a1(k)=alpha(k)
   ...
   20   continue
        x=z(imu)
        call schri(mi,7,a1,b1,x,0.,0.,0.,alpha,beta,ierrc)
   30 continue
      return
      end
