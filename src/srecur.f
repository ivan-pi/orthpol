...
c
   ...
   60   continue
        ...
        if(al.le.-1. .or. be.le.-1.) then
          ...
          alpbe=al+be
          a(1)=(be-al)/(alpbe+2.)
          t=(alpbe+1.)*log(2.)+salga(al+1.)+salga(be+1.)-
     *      salga(alpbe+2.)
          if(t.gt.almach) then
            ierr=2
            b(1)=r1mach(2)
...
c 
     ...
     *.4097792921092615/
c
c The constants in the statement below are  exp(1.)  and  .5*log(8.).
c
...
c function subroutines  salga  and  r1mach. In case of overflow, the
